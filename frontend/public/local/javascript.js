(function () {
  ...
  if (typeof window.rrweb === "undefined") {
    console.error(
      "rrweb is not loaded. Please include rrweb script before this recorder."
    );
    return;
  }
        ...
        try {
          if (
            ...
            responseBody = `[Binary response: ${
              xhr.response?.size || xhr.response?.byteLength || "unknown"
            } bytes]`;
            responseType = "binary";
          } else {
  ...
  const emitNav = () =>
    addCustomEvent("nav", { href: location.href, t: Date.now() });
  ...
  window.__rr = {
    ...
    getNetworkEvents: () => {
      return session.filter(
        (event) => event.type === 5 && event.data.tag === "network"
      );
    },
  };
  ...
  if (sessionStorage.getItem("rrweb_testmode") === "true") {
    ...
    function createTestModeUI() {
      ...
      timeDisplay.style.cssText =
        "color:#FFF;font-family:'NDOT 47', 'Courier New', monospace;font-size:16px;font-style:normal;font-weight:400;line-height:20px;letter-spacing:-0.32px;opacity:0.7";
